...
| 🎮 **GPU（显卡）** | 展示 GPU 使用率、核心温度及显存占用情况，兼容 NVIDIA / AMD / Intel 显卡。 |
| 💾 **内存（Memory）** | 显示系统内存占用率，支持 GB 单位与百分比显示，轻松查看内存负载。 |
| 📀 **磁盘（Disk）** | 监控磁盘读取与写入速度（KB/s、MB/s），帮助分析 I/O 负载情况。 |
| 🌐 **网络（Network）** | 实时显示上传与下载速度（KB/s、MB/s），轻量级网络流量监控。 |
